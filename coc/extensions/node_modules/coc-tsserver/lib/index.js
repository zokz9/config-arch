...
var _TagClosing = class _TagClosing {
  constructor(client, descriptionLanguageId) {
    ...
    import_coc27.workspace.onDidChangeConfiguration((e) => {
    ...
    this._timeout = setTimeout(async () => {
      ...
      if (doc.changedtick === changedtick) {
        ...
        if (!nowarning) {
          ...
          let res = await import_coc27.window.showWarningMessage(
            `Automatic inserted tag by coc-tsserver, use configuration "${filetype}.autoClosingTags": false to disable this behavior`,
            ...items
          );
          if (res == null) return;
          if (res.id == 1) {
            let config = import_coc27.workspace.getConfiguration(filetype, null);
...
var TsserverService = class {
   ...
   */
  getClientHost() {
    if (this.state == import_coc48.ServiceStat.Running) return Promise.resolve(this.clientHost);
    this.start();
    return new Promise((resolve, reject) => {
  ...
  async start() {
    if (!this.enable || this._state == import_coc48.ServiceStat.Starting) return;

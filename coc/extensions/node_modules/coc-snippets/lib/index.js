...
var ProviderManager = class {
  ...
  async init() {
    ...
    let promises = [];
    import_coc5.workspace.documents.forEach((doc) => {
      let filetype = getSnippetFiletype(doc);
      promises.push(this.loadSnippetsByFiletype(filetype));
    });
    import_coc5.workspace.onDidOpenTextDocument(async (doc) => {
...
` + e.stack;
  ...
  async executePythonCode(pythonCode) {
    ...
    try {
      ...
      import_coc8.window.showErrorMessage(`Error on execute python script: ${e.message}`);
    }
  }
};
...
function checkBufferVariable(doc) {
  ...
  if (!filetypes) {
    ...
    if (arr)
      doc.buffer.setVar("coc_snippets_filetypes", arr, true);

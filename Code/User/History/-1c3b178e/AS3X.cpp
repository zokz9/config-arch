...
void processInput(GLFWwindow* window) {
    ...
    // Сохраняем начальную позицию для коррекции
    ...
    if (moved) {
        ...
        glm::vec3 toCube = cameraPos - cubeCenter;
        float distance = glm::length(toCube);
        float radius = 0.8f; // Радиус куба (длина стороны)

        if (distance < radius) {
            // Направление от куба к камере
            glm::vec3 dir = glm::normalize(toCube);
            // Возвращаем камеру на границу радиуса
            cameraPos = cubeCenter + dir * radius;
        }
    }
}

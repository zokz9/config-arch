...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        switch (key) {
            case GLFW_KEY_A: cameraPos.x -= speed; break;
            case GLFW_KEY_D: cameraPos.x += speed; break;
            case GLFW_KEY_W: cameraPos.z -= speed; break;
            case GLFW_KEY_S: cameraPos.z += speed; break;
            case GLFW_KEY_Q: cameraPos.y += speed; break;
            case GLFW_KEY_E: cameraPos.y -= speed; break;
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 800, "3D Cube with Camera", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }

...
const char* fragmentShaderSource = R"glsl(
    #version 330 core
    out vec4 FragColor;
    uniform vec3 color; // Добавляем uniform для цвета
    
    void main() {
        FragColor = vec4(color, 1.0);  // Используем переданный цвет
    }
)glsl";
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        switch (key) {
            ...
            case GLFW_KEY_S:  // Клавиша S - движение вниз
                offsetY -= speed;
                break;
            case GLFW_KEY_LEFT:  // Стрелка влево - вращение против часовой
                rotationAngle += rotationSpeed;
                break;
            case GLFW_KEY_RIGHT:  // Стрелка вправо - вращение по часовой
                rotationAngle -= rotationSpeed;
                break;
        }
    }
...
int main() {
    ...
    // Создание окна
    GLFWwindow* window = glfwCreateWindow(1300, 800, "2D OpenGL Triangle", NULL, NULL);
    if (!window) {
    ...
    float vertices[] = {
        -0.2f, -0.2f,  // Левая нижняя
        0.2f, -0.2f,  // Правая нижняя
        0.2f,  0.2f,   // Верхняя
        0.2f, 0.2f,
        -0.2f,0.2f,
        -0.2f,-0.2f
    };
        // Координаты вершин квадрата (основа лица)
    float faceVertices[] = {
        // Квадрат (лицо) - 2 треугольника
        -0.2f, -0.2f,  // Левая нижняя
        0.2f, -0.2f,  // Правая нижняя
        0.2f,  0.2f,  // Правая верхняя
        0.2f,  0.2f,  // Правая верхняя
        -0.2f,  0.2f,  // Левая верхняя
        -0.2f, -0.2f   // Левая нижняя
    };

    // Координаты вершин глаз (два маленьких квадрата)
    float eyeVertices[] = {
        // Левый глаз (2 треугольника)
        -0.1f, 0.1f,   // Левый нижний
        -0.05f, 0.1f,  // Правый нижний
        -0.05f, 0.15f, // Правый верхний
        -0.05f, 0.15f, // Правый верхний
        -0.1f, 0.15f,  // Левый верхний
        -0.1f, 0.1f,   // Левый нижний

        // Правый глаз (2 треугольника)
        0.05f, 0.1f,   // Левый нижний
        0.1f, 0.1f,    // Правый нижний
        0.1f, 0.15f,   // Правый верхний
        0.1f, 0.15f,   // Правый верхний
        0.05f, 0.15f,  // Левый верхний
        0.05f, 0.1f    // Левый нижний
    };

    // Координаты рта (линия или прямоугольник)
    float mouthVertices[] = {
        // Рот (прямоугольник из 2 треугольников)
        -0.1f, -0.1f,   // Левый нижний
        0.1f, -0.1f,   // Правый нижний
        0.1f, -0.05f,  // Правый верхний
        0.1f, -0.05f,  // Правый верхний
        -0.1f, -0.05f,  // Левый верхний
        -0.1f, -0.1f    // Левый нижний
    };
    ...
    for (int i = 0; i < 5; i += 2) { // Проходим по всем x-координатам (каждый второй элемент)
...
GLuint eyesVAO, eyesVBO;
GLuint mouthVAO, mouthVBO;

    // Настройка VAO и VBO для лица
    ...
    // Настройка VAO и VBO для рта
    glGenVertexArrays(1, &mouthVAO);
    glGenBuffers(1, &mouthVBO);
    glBindVertexArray(mouthVAO);
    glBindBuffer(GL_ARRAY_BUFFER, mouthVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(mouthVertices), mouthVertices, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Создаем матрицу трансформации (сначала перемещение, затем вращение)
        transform = glm::mat4(1.0f); // Единичная матрица
        transform = glm::translate(transform, glm::vec3(offsetX, offsetY, 0.0f));  // Перемещение
        transform = glm::rotate(transform, glm::radians(rotationAngle), glm::vec3(0.0f, 0.0f, 1.0f));  // Вращение
        ...
        glDrawArrays(GL_TRIANGLES, 0, 6);
         // Получаем location uniform-переменной цвета
        GLint colorLoc = glGetUniformLocation(shaderProgram, "color");
        // Применяем трансформацию (одинаковую для всех элементов лица)
        ...
        // Рисуем лицо (желтый квадрат)
        glUniform3f(colorLoc, 1.0f, 1.0f, 0.2f); // Желтый
        glBindVertexArray(faceVAO);
        glDrawArrays(GL_TRIANGLES, 0, 6);

        // Рисуем глаза (черные)
        glUniform3f(colorLoc, 0.0f, 0.0f, 0.0f); // Черный
        glBindVertexArray(eyesVAO);
        glDrawArrays(GL_TRIANGLES, 0, 12); // 12 вершин (2 глаза по 6 вершин)

...
// ▶️ Цвет куба (можно менять здесь)
glm::vec3 cubeColor = glm::vec3(0.5f, 0.8f, 1.0f); // Голубой (RGB)
...
const char* fragmentShaderSource = R"glsl(
    ...
    void main() {
        // ▶️ Направление света (от трёх октант)
        vec3 lightDir = normalize(vec3(1.0, 1.0, -1.0));

        // ▶️ Освещённость (градус между нормалью и светом)
        float diff = max(dot(Normal, lightDir), 0.0);

        // ▶️ Окончательный цвет = цвет куба с освещением
        FragColor = vec4(triangleColor * diff, 1.0);
    }
)glsl";
...
std::vector<Triangle> generateTrianglesInfo(const std::vector<Vertex>& vertices) {
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        float distance = glm::dot(cameraPos - center, triangleNormal);
        triangles.push_back({i, distance, center});
    }
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 800, "3D Рендеринг Куба", nullptr, nullptr);
    if (!window) {
        glfwTerminate();
        return -1;
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform3fv(colorLoc, 1, glm::value_ptr(cubeColor)); // Устанавливаем единственный цвет
